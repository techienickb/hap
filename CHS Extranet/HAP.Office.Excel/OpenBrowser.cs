...
namespace HAP.Office.Excel
{
    public partial class OpenBrowser : Form
    {
        ...
        private void LoadFile()
        {
            ...
            if (ret != null || ret != string.Empty)
            {
                ...
                this.BeginInvoke(new Action(() =>
                {
                    ...
                    {
                        if (file.Extension == "" || Regex.IsMatch(file.Extension, ".*\\.(xls||xlsx||xltx||xlt)$", RegexOptions.IgnoreCase))
        ...
        private void listView1_ItemActivate(object sender, EventArgs e)
        {
            ...
            if (temppath.StartsWith(".."))
            {
                if (SaveMode)
                {
                    ...
                    fileType.SelectedIndex = Regex.IsMatch(listView1.SelectedItems[0].SubItems[4].Text, ".*\\.(xls||xlsx)") ? 0 : 1;
        ...
        private void Download(object o)
        {
            ...
            this.Invoke(new Action(() =>
            {
                ...
                try
                {
                    ...
                    Globals.ThisAddIn.Application.WorkbookBeforeClose += Application_BeforeClose;
                    ExcelSaveHandler wsh = new ExcelSaveHandler(Globals.ThisAddIn.Application);
        ...
        void wsh_AfterUiSaveEvent(Excel2.Workbook Wb, bool isClosed)
        {
            SaveAsd = true;
            if (SaveMode)
            {
        ...
        void Application_BeforeClose(Excel2.Workbook Wb, ref bool Cancel)
        {
            if (!SaveAsd && ! Wb.Saved && MessageBox.Show("If you wish to Save the changes and upload them back to HAP+, please use the File > Save function first before closing.\nDo you still wish to close?", "Question", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.No) Cancel = true;
        }

        void wsh_AfterSaveEvent(Excel2.Workbook Wb, bool isClosed)
        {
            if (!SaveAsd)
            {
                new Saving(filename, path, token).ShowDialog();
        ...
        void ThisAddIn_Shutdown(object sender, EventArgs e)
        {
            if (File.Exists(filename))
                ...
                catch { Thread.Sleep(10); try { File.Delete(filename); } catch { } }
        }
        ...
        private void CheckFile(object o)
        {
            ...
            string ret = string.Empty;
            path = path.Replace('\\', '/');
            string p2 = path + (path.EndsWith("/") ? "" : "/") + os[0] + "." + (os[1] == "0" ? "xlsx" : "xltx");
            ...
            if (ret != null || ret != string.Empty)
            {
                ...
                if (p.Name == null || MessageBox.Show("Do you want to overwrite this file?", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                    this.BeginInvoke(new Action(() =>
                    {
                        filename = System.IO.Path.Combine(System.IO.Path.GetTempPath(), os[0] + "." + (os[1] == "0" ? "xlsx" : "xltx"));
                        path = "./api/myfiles-upload/" + path + (path.EndsWith("/") ? "" : "/") + os[0] + "." + (os[1] == "0" ? "xlsx" : "xltx");
                        Globals.ThisAddIn.Application.WorkbookBeforeClose += Application_BeforeClose;
                        ExcelSaveHandler wsh = new ExcelSaveHandler(Globals.ThisAddIn.Application);
                        wsh.AfterSaveEvent += wsh_AfterSaveEvent;
                        wsh.AfterUiSaveEvent += wsh_AfterUiSaveEvent;
                        Globals.ThisAddIn.Shutdown += ThisAddIn_Shutdown;
                        Globals.ThisAddIn.Application.ActiveWorkbook.SaveAs(filename, AddToMru: false);
                        SaveAsd = false;
                        Close();
                    }));
                else
